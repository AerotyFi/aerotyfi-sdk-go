...
const defaultBaseURL = "https://api.privenoxfi.com"
const defaultWsURL = "wss://ws.privenoxfi.com"
const defaultTimeout = 30_000 // milliseconds

// privenoxClient is the primary client for the privenox Fi API.
type privenoxClient struct {
	config       privenoxConfig
...
// NewClient creates a new privenoxClient with the given configuration.
...
// NewClientFromEnv creates a new privenoxClient using environment variables.
// privenoxfi_API_KEY is required. Optional: privenoxfi_BASE_URL, privenoxfi_WS_URL, privenoxfi_TIMEOUT.
func NewClientFromEnv() (*privenoxClient, error) {
	apiKey, _ := os.LookupEnv("privenoxfi_API_KEY")
	if apiKey == "" {
		return nil, fmt.Errorf("privenoxfi: privenoxfi_API_KEY environment variable is required")
	}

	timeout := defaultTimeout
	if t, ok := os.LookupEnv("privenoxfi_TIMEOUT"); ok {
		if _, err := fmt.Sscanf(t, "%d", &timeout); err != nil {
			return nil, fmt.Errorf("privenoxfi: invalid privenoxfi_TIMEOUT: %s", t)
		}
	}

	baseURL, _ := os.LookupEnv("privenoxfi_BASE_URL")
	wsURL, _ := os.LookupEnv("privenoxfi_WS_URL")

	return NewClient(privenoxConfig{
		APIKey:  apiKey,
		BaseURL: baseURL,
		WsURL:   wsURL,
		Timeout: timeout,
...
func (c *privenoxClient) request(ctx context.Context, method, path string, body interface{}, result interface{}) error {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if body != nil {
			...
			if err != nil {
				return fmt.Errorf("privenoxfi: marshal request body: %w", err)
			}
		...
		if err != nil {
			return fmt.Errorf("privenoxfi: create request: %w", err)
		}
		...
		if err != nil {
			...
			return fmt.Errorf("privenoxfi: request failed after %d attempts: %w", maxRetries+1, lastErr)
		}
		...
		if err != nil {
			if attempt < maxRetries {
				lastErr = fmt.Errorf("privenoxfi: read response: %w", err)
				continue
			}
			return fmt.Errorf("privenoxfi: read response: %w", err)
		}

		if resp.StatusCode >= 400 {
			var apiErr privenoxError
			if err := json.Unmarshal(respBytes, &apiErr); err != nil {
				apiErr = privenoxError{
					Code:    "unknown",
		...
		if err := json.Unmarshal(respBytes, result); err != nil {
			return fmt.Errorf("privenoxfi: unmarshal response: %w", err)
		}
		return nil
	}

	if lastErr != nil {
		if apiErr, ok := lastErr.(privenoxError); ok {
			return apiErr
		}
		return fmt.Errorf("privenoxfi: request failed after retries: %w", lastErr)
	}
	return fmt.Errorf("privenoxfi: request failed after retries")
}
...
func generateIdempotencyKey() string {
	b := make([]byte, 16)
	_, _ = rand.Read(b)
	return "privenox-" + hex.EncodeToString(b)
}

// ── Health ──────────────────────────────────────────────────────

// Health checks the API /v1/health endpoint.
func (c *privenoxClient) Health(ctx context.Context) (HealthResponse, error) {
...
func (c *privenoxClient) ListAllAccounts(ctx context.Context, limit int) ([]Account, error) {
	var all []Account
	cursor := ""
	for {
...
func (c *privenoxClient) ListCards(ctx context.Context, accountID string, params PaginationParams) (PaginatedResponse[Card], error) {
...
func (c *privenoxClient) ListAllCards(ctx context.Context, accountID string, limit int) ([]Card, error) {
	var all []Card
	cursor := ""
	for {
		params := PaginationParams{Limit: limit, Cursor: cursor}
...
func (c *privenoxClient) ListAgents(ctx context.Context, accountID string, params PaginationParams) (PaginatedResponse[Agent], error) {
...
func (c *privenoxClient) ListAllAgents(ctx context.Context, accountID string, limit int) ([]Agent, error) {
	var all []Agent
	cursor := ""
	for {
		params := PaginationParams{Limit: limit, Cursor: cursor}
