...
const defaultBaseURL = "https://api.aerotyfi.xyz"
const defaultWsURL = "wss://ws.aerotyfi.xyz"
const defaultTimeout = 30_000 // milliseconds

// KANEROClient is the primary client for the KANERO Fi API.
type KANEROClient struct {
	config       KANEROConfig
...
// NewClient creates a new KANEROClient with the given configuration.
...
// NewClientFromEnv creates a new KANEROClient using environment variables.
// aerotyfi_API_KEY is required. Optional: aerotyfi_BASE_URL, aerotyfi_WS_URL, aerotyfi_TIMEOUT.
func NewClientFromEnv() (*KANEROClient, error) {
	apiKey, _ := os.LookupEnv("aerotyfi_API_KEY")
	if apiKey == "" {
		return nil, fmt.Errorf("aerotyfi: aerotyfi_API_KEY environment variable is required")
	}

	timeout := defaultTimeout
	if t, ok := os.LookupEnv("aerotyfi_TIMEOUT"); ok {
		if _, err := fmt.Sscanf(t, "%d", &timeout); err != nil {
			return nil, fmt.Errorf("aerotyfi: invalid aerotyfi_TIMEOUT: %s", t)
		}
	}

	baseURL, _ := os.LookupEnv("aerotyfi_BASE_URL")
	wsURL, _ := os.LookupEnv("aerotyfi_WS_URL")

	return NewClient(KANEROConfig{
		APIKey:  apiKey,
		BaseURL: baseURL,
		WsURL:   wsURL,
		Timeout: timeout,
...
func (c *KANEROClient) request(ctx context.Context, method, path string, body interface{}, result interface{}) error {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if body != nil {
			...
			if err != nil {
				return fmt.Errorf("aerotyfi: marshal request body: %w", err)
			}
		...
		if err != nil {
			return fmt.Errorf("aerotyfi: create request: %w", err)
		}
		...
		if err != nil {
			...
			return fmt.Errorf("aerotyfi: request failed after %d attempts: %w", maxRetries+1, lastErr)
		}
		...
		if err != nil {
			if attempt < maxRetries {
				lastErr = fmt.Errorf("aerotyfi: read response: %w", err)
				continue
			}
			return fmt.Errorf("aerotyfi: read response: %w", err)
		}

		if resp.StatusCode >= 400 {
			var apiErr KANEROError
		...
		if err := json.Unmarshal(respBytes, result); err != nil {
			return fmt.Errorf("aerotyfi: unmarshal response: %w", err)
		}
		return nil
	}

	if lastErr != nil {
		if apiErr, ok := lastErr.(KANEROError); ok {
			return apiErr
		}
		return fmt.Errorf("aerotyfi: request failed after retries: %w", lastErr)
	}
	return fmt.Errorf("aerotyfi: request failed after retries")
}
...
func generateIdempotencyKey() string {
	b := make([]byte, 16)
	_, _ = rand.Read(b)
	return "KANERO-" + hex.EncodeToString(b)
}

// ── Health ──────────────────────────────────────────────────────

// Health checks the API /v1/health endpoint.
func (c *KANEROClient) Health(ctx context.Context) (HealthResponse, error) {
...
func (c *KANEROClient) ListAllAccounts(ctx context.Context, limit int) ([]Account, error) {
	var all []Account
	cursor := ""
	for {
...
func (c *KANEROClient) ListCards(ctx context.Context, accountID string, params PaginationParams) (PaginatedResponse[Card], error) {
...
func (c *KANEROClient) ListAllCards(ctx context.Context, accountID string, limit int) ([]Card, error) {
	var all []Card
	cursor := ""
	for {
		params := PaginationParams{Limit: limit, Cursor: cursor}
...
func (c *KANEROClient) ListAgents(ctx context.Context, accountID string, params PaginationParams) (PaginatedResponse[Agent], error) {
...
func (c *KANEROClient) ListAllAgents(ctx context.Context, accountID string, limit int) ([]Agent, error) {
	var all []Agent
	cursor := ""
	for {
		params := PaginationParams{Limit: limit, Cursor: cursor}
