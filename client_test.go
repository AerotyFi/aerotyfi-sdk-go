...
func TestNewClientDefaults(t *testing.T) {
	c := NewClient(KANEROConfig{APIKey: "test-key"})
...
func TestNewClientCustomConfig(t *testing.T) {
	c := NewClient(KANEROConfig{
		APIKey:  "custom-key",
		BaseURL: "https://custom.api.xyz",
		WsURL:   "wss://custom.ws.xyz",
		Timeout: 5000,
	})
	if c.config.BaseURL != "https://custom.api.xyz" {
...
func TestIdempotencyKeyGeneration(t *testing.T) {
	...
	if len(key1) < 6 {
		t.Errorf("key should have prefix 'KANERO-', got %q", key1)
	}
}
...
func TestHealth(t *testing.T) {
	...
	c := NewClient(KANEROConfig{APIKey: "test-key", BaseURL: server.URL})
...
func TestCreateAccountError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(KANEROError{
			Code:    "unauthorized",
	...
	c := NewClient(KANEROConfig{APIKey: "bad-key", BaseURL: server.URL})
	...
	apiErr, ok := err.(KANEROError)
	if !ok {
		t.Fatalf("expected KANEROError, got %T: %v", err, err)
	}
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewEncoder(w).Encode(PaginatedResponse[Account]{
			Data: []Account{
				...
				{ID: "acc_2", Label: "trading", Asset: AssetETH},
			},
			Cursor:  "cursor_next",
			HasMore: true,
		})
...
func TestSwap(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var req SwapRequest
		json.NewDecoder(r.Body).Decode(&req)
		if req.From != AssetETH || req.To != AssetUSDC {
			t.Errorf("expected ETH->USDC, got %s->%s", req.From, req.To)
		}
...
func TestKANEROErrorImplementsError(t *testing.T) {
	err := KANEROError{Code: "test", Message: "test msg", Status: 400}
	if err.Error() == "" {
...
func TestRetryOn5xx(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if attempts < 3 {
			...
			w.Header().Set("Content-Type", "application/json")
			json.NewEncoder(w).Encode(KANEROError{Code: "INTERNAL", Message: "boom", Status: 500})
			return
		}
		w.Header().Set("Content-Type", "application/json")
...
func TestNoRetryOn4xx(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(KANEROError{Code: "BAD_REQUEST", Message: "invalid", Status: 400})
	}))
	defer server.Close()
